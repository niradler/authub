...
from authub.models import IdentityProvider, OAuth2Settings
from authub.protocols.oauth2 import OAuth2Protocol


def make_conn(userinfo: bool = True) -> IdentityProvider:
...
async def test_begin_has_state_and_pkce() -> None:
    ...
    result = await protocol.begin(idp=make_conn(), callback_url="http://app/cb", return_to="/")
...
async def test_complete_without_userinfo_uses_token_response() -> None:
    ...
    begin = await protocol.begin(idp=conn, callback_url="http://app/cb", return_to="/")
    request = get_request(f"http://app/cb?code=c1&state={begin.flow_state.state}")
    raw = await protocol.complete(
        request=request, idp=conn, callback_url="http://app/cb", flow_state=begin.flow_state
    )
